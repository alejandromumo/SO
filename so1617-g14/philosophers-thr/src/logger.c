...
static pthread_mutex_t LOGGER_ACCESS = PTHREAD_MUTEX_INITIALIZER;
...
static void invariantCheck(Simulation* sim)
{
	...
	{
	  fprintf(stderr, "INVARIANT ERROR: Invalid buffet number of spaghetti: %d", sim->diningRoom->spaghetti);
	  exit(EXIT_FAILURE);
	}
	if (!(sim->diningRoom->cleanForks >= 0 && sim->diningRoom->cleanForks <= sim->params->NUM_FORKS))
	{
	  fprintf(stderr, "INVARIANT ERROR: Invalid buffet number of clean forks: %d", sim->diningRoom->cleanForks);
	  exit(EXIT_FAILURE);
	}
	if (!(sim->diningRoom->cleanKnives >= 0 && sim->diningRoom->cleanKnives <= sim->params->NUM_KNIVES))
	{
	  fprintf(stderr, "INVARIANT ERROR: Invalid buffet number of clean knives: %d", sim->diningRoom->cleanKnives);
	  exit(EXIT_FAILURE);
	}
	...
	{
	  fprintf(stderr, "INVARIANT ERROR: Invalid buffet number of dirty forks: %d", sim->diningRoom->dirtyForks+sim->diningRoom->dirtyForksInWaiter);
	  exit(EXIT_FAILURE);
	}
	...
	{
	  fprintf(stderr, "INVARIANT ERROR: Invalid buffet number of dirty knives: %d", sim->diningRoom->dirtyKnives+sim->diningRoom->dirtyKnivesInWaiter);
	  exit(EXIT_FAILURE);
	}
}
...
static void unsafe_logger(Simulation* sim)
{
	...
	int dirtyForks = sim->diningRoom->dirtyForks + sim->diningRoom->dirtyForksInWaiter;
	int dirtyKnives = sim->diningRoom->dirtyKnives + sim->diningRoom->dirtyKnivesInWaiter;
